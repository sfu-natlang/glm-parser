...
class GlmParser():
    def __init__(self,
        ...
        if parallelFlag:
            self.learner = importlib.import_module(
                    'learn.' + learner).Learner()
        else:
            self.learner = importlib.import_module(
                    'learn.' + learner).Learner(self.w_vector)
    ...
    def train(self,
        ...
        if not isinstance(dataPool, DataPool):
            raise ValueError("PARSER [ERROR]: dataPool\
                             for training is not an DataPool object")
        ...
        def parser_f_argmax(w_vector, sentence, parser):
            current_edge_set = parser.parse(sentence,
                                            w_vector.get_vector_score)
        ...
        if not parallel:  # using sequential training
            logger.debug("Using Sequential Training")
            ...
            logger.debug("Using Parallel Training")
            if shardNum is None:
                logger.warn("Number of shards not specified, using 1")
                shardNum = 1
            if sc is None:
                raise RuntimeError(
                        'PARSER [ERROR]: SparkContext not specified')

            parallelLearnClass = importlib.import_module(
                    'learn.spark_train').ParallelPerceptronLearner
    ...
    def evaluate(self, dataPool=None, tagger=None):
        if not isinstance(dataPool, DataPool):
            raise ValueError("PARSER [ERROR]: dataPool\
                             for evaluation is not an DataPool object")
...
if __name__ == "__main__":
    ...
    if True:  # Adding arguments
        arg_parser = argparse.ArgumentParser(
                description='Global Linear Model (GLM) Parser\
                Version %s' % __version__)
        arg_parser.add_argument(
            'config', metavar='CONFIG_FILE', nargs='?',
            help="""specify the config file. This will load all the setting
            from the config file, in order to avoid massive command line
            inputs.  Please consider using config files instead of manually
            typing all the options.

            Additional options by command line will override the settings in
            the config file.

            Officially provided config files are located in src/config/
            """)
        arg_parser.add_argument(
            '--train', metavar='TRAIN_FILE_PATTERN',
            help="""specify the data for training with regular expression""")
        arg_parser.add_argument(
            '--test', metavar='TEST_FILE_PATTERN',
        ...
        arg_parser.add_argument(
            ...
            (mandatory).  The file will be searched under /feature directory,
            and the class object that has a get_local_vector() interface will
            be recognised as the feature generator and put into use
            automatically.

            If multiple eligible objects exist, an error will be reported.

            For developers: please make sure there is only one such class
            object under fgen source files. Even import statement may introduce
            other modules that is eligible to be an fgen. Be careful.
        ...
        arg_parser.add_argument(
            ...
            help="""specify the parser using parser module name (i.e. .py file
            name without suffix). The recognition rule is the same as
            --fgen switch. A valid parser object must possess "parse" attribute
            in order to be recognised as a parser implementation.

            Some parser might not work correctly with the infrastructure, which
            keeps changing all the time. If this happens please file an issue
            on github page

            default "ceisner"; alternative "ceisner3"
            """)
        arg_parser.add_argument(
            '--format', metavar='FORMAT_FILE',
            ...
            """)
        arg_parser.add_argument(
            '--max-sentences', metavar='N', type=int,
            help="""run the first [int] sentences only. Usually combined with
            option -a to gather time usage information
            If combined with -a, then time usage information is available, and
            it will print out average time usage after each iteration
            *** Caution: Overrides -t (no evaluation will be conducted), and
            partially overrides -b -e (Only run specified number of sentences)
            -i (Run forever)
            """)
        arg_parser.add_argument(
            '--log-feature-request', action='store_true',
            ...
            """)
        arg_parser.add_argument(
            '--spark-shards', '-s', metavar='SHARDS_NUM',
        ...
        arg_parser.add_argument(
            ...
            will be on HDFS.""")
        arg_parser.add_argument(
            '--data-path', '-p', metavar='DATA_PATH',
            help="""Path to data files (to the parent directory for all
            sections)
            default "./penn-wsj-deps/" """)
        arg_parser.add_argument(
            '--load-weight-from', '-l', metavar='FILENAME',
            help="""Path to an existing weight vector dump file
            example: "./Weight.db" """)
        arg_parser.add_argument(
            '--dump-weight-to', '-d', metavar='PREFIX',
        ...
        arg_parser.add_argument(
            '--iterations', '-i', metavar='ITERATIONS',
            type=int, help="""Number of iterations
            default 1""")
        arg_parser.add_argument(
            '--timing', '-a', action='store_true',
    ...
    if args.max_sentences:
        ...
        if debug.debug.run_first_num <= 0:
            ...
            logger.debug("run number = %d" % (debug.debug.run_first_num, ))
    if args.log_feature_request:
        debug.debug.log_feature_request_flag = True
        logger.debug("Enable feature request log")
    if args.timing:
    ...
    if args.config:
        ...
        if (not os.path.isfile(args.config)) and (not yarn_mode):
            sys.stderr.write(
                    "The config file doesn't exist: %s\n" % args.config)
            sys.exit(1)
        ...
        for option in ['load_weight_from',
                       'dump_weight_to',
                       'tagger_w_vector']:
    ...
    if config['train']:
        ...
        logger.info("Training time usage(seconds): %f" % training_time)
