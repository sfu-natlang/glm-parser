...
def get_maxvalue(viterbi_dict):
    maxvalue = ((None,None), None) # maxvalue has tuple (tag, value)
    for (tag,prev) in viterbi_dict.keys():
        value = viterbi_dict[(tag,prev)] # value is (score, backpointer)
        if maxvalue[1] is None:
            maxvalue = ((tag,prev), value[0])
...
def perc_test(feat_vec, labeled_list, tagset, default_tag):
    ...
    for i in range(2, N-2):
        ...
        for tag in tagset:
            ...
            prev_list = []
            for (prev_tag_1, prev_tag_2) in viterbi[i-1]:
                feats = []
                #(prev_value, prev_backpointer_a, prev_backpointer_b) = viterbi[i-1][(prev_tag_1, prev_tag_2)]
                prev_value = viterbi[i-1][(prev_tag_1, prev_tag_2)][0]
                pos_feat.get_pos_feature(feats,i,prev_tag_1,prev_tag_2)
                weight = 0.0
                # sum up the weights for all features except the bigram features
                for feat in feats:
                #if feat == 'B': has_bigram_feat = True
            ...
            #         print >>sys.stderr, "feat:", feat, "tag:", tag, "weight:", feat_vec[feat, tag]
                ...
                prev_list.append( (prev_tag_weight + prev_value, prev_tag_1, prev_tag_2) )
    ...
    best_tag, prev = maxvalue[0]
    for i in range(N-3, 1, -1):
        output.insert(0,best_tag)
        (value, prev, prev2) = viterbi[i][(best_tag,prev)]
        best_tag = prev
        prev = prev2
