...
class Pos_feat_gen():
	...
	def get_pos_feature(self,fv, i, pretag_1, pretag_2, pretag_3):
		...
		#fv.append((19,pretag_3))
		#if len(self.wl[i-1])>=3:
		fv.append((20,self.wl[i-1].lower()[-3:]))
		#if len(self.wl[i+1])>=3:
		fv.append((21,self.wl[i+1].lower()[-3:]))
		fv.append((22,pretag_3))
		fv.append((23,pretag_3,pretag_2, pretag_1))
		fv.append((24,"bias"))
		#fv.append((25,pretag_1,word))
	...
	def get_sent_feature(self, fv, poslist):
		for i in range(3, len(self.wl)-2):
			...
			#fv[(19,poslist[i-3]),tag]+=1
			#if len(self.wl[i-1])>=3:
			fv[(20,self.wl[i-1].lower()[-3:]),tag]+=1
			#if len(self.wl[i+1])>=3:
			fv[(21,self.wl[i+1].lower()[-3:]),tag]+=1
			fv[(22,poslist[i-3]),tag]+=1
			fv[(23,poslist[i-3],poslist[i-2],poslist[i-1]),tag]+=1
			fv[(24,"bias"),tag]+=1
